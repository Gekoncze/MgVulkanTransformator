...
public class FunctionParser implements Parser {
    ...
    */
    ...
    public CEntity parse(ChainList<String> lines, int i) {
        ...
        if(!line.startsWith("    ")){
            if(line.startsWith("typedef ") && line.contains("(VKAPI_PTR *PFN_") && !line.endsWith(")(")){
                ...
                for(String part : ps){
                    CParameter p = (parseParameter(part));
    ...
    public static CParameter parseReturn(String part){
        int pointerCount = StringUtilities.count(part, '*');
        part = part.replace("*", " ").trim();
        return new CParameter(part, "rval", pointerCount);
    }
    ...
    public static CParameter parseParameter(String part){
        ...
        return new CParameter(parts2[0], parts3[0], pointerCount);
    }
}
